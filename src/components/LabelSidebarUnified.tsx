...
import { Plus, Tag, Trash2, CheckCircle2, Circle } from "lucide-react";
...
interface LabelSidebarUnifiedProps {
  labels: Label[];
...
export const LabelSidebarUnified = ({
  labels,
  selectedLabelId,
  onSelectLabel,
  onAddLabel,
    ...
    <div className="w-80 border-r bg-card flex flex-col h-full overflow-hidden">
      ...
      <ScrollArea className="flex-1">
        <div className="p-2 space-y-1">
          {labels.map((label) => {
            const isTagged = tags.some((t) => t.labelId === label.id);
